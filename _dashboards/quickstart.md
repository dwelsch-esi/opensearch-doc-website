...
- <http://localhost:4000/latest/dashboards/quickstart/> for a local installation.
...
- Choose an index pattern for analysis
- Filter the chosen indexes by a time range, and by restricting data using the UI or [Dashboards Query Language (DQL)]({{site.url}}{{site.baseurl}}/dashboards/dql/)
- View the selected documents in a table
- Examine individual documents
- Create histograms to display the distribution of your data

Follow these steps to explore the example flight data using the Discover tool:
...
1. Select **Update**.
    ...
    Documents are listed for US-bound flights delayed by 60 minutes or more.
...
With the **Dashboards** application, you create and save collections of visualizations.

Follow these steps to view a dashboard in the **Dashboards** application:
...
1. In the **search toolbar**, search for and select **[Flights] Global Flight Dashboard**. 
    ...
    The **panel** displays a dashboard of visualizations, including charts, maps, and data tables.

To add another visualization to the dashboard:

1. select the **Edit** button.

1. Choose **Add** from the toolbar.
